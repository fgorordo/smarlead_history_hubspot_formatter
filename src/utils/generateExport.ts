...
const getAllEmailsFromCampaign = async (leads: Lead[], campaignId: number) => {
...
export const generateExport = async (selection: number) => {
    ...
    const emails = await await getAllEmailsFromCampaign(leads, selection)
    createExportFile("Contacts",leads,"leads");
